...
def groupFiles_merge(grouppath):
    ...
    print("{}下共找到{}个rebuild_csv,开始merge......".format(grouppath,len(filesList)))
    for file in filesList:
        if("_rebuild" in file):
            df = pd.read_csv(file)
            df.to_csv(grouppath + '\\merge.csv', encoding="utf_8_sig", index=False, header=False, mode='a+')

    print("merge成功，路径为{}".format(grouppath + '\\merge.csv'))
            ...
            for i in range(len(row)):
                if(i == 0):
                    ...
                    if not (pd.isna(row[i])):
                        if(len(item) == 1):
                            item.append(str(row[i]))
                        else:
                            item[1] = item[1]+";"+str(row[i])

                    else:
                        break
            # print("第{}行=>处理结束".format(index))
            if(len(item)>1):
                items.append(item)
                count+=1
        if(len(items)>0):
