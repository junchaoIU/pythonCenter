...
inertias = model_kmeans.inertia_  # 样本到最近的聚类中心的距离总和
...
calinski_harabaz_s = metrics.calinski_harabasz_score(X, y_pre)  # Calinski和Harabaz得分
...
print('%d\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%d' % (
    ...
    v_measure_s,silhouette_s, calinski_harabaz_s))  # 打印输出指标值
print(70 * '-')  # 打印分隔线
